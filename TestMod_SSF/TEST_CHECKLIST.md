...
- [ ] `14-M05` 변신 규율: 절대적으로 혐오함 → **자기 주도 변신 금지** (기즈모 비활성 "사상이 금지함", 약물 섭취 불가, 주문서 사용 불가), **타인에 의한 강제 변신은 허용** (원래 시간 유지), 강제 변신 시 기분 -10, 의견 -20, 기억 감정 -10 (5일), **목격 기억 감정 -8 (5일)**
- [ ] `14-M05a` 변신 규율: 절대적으로 혐오함 — 약물 우클릭 시 "(사상이 금지함)" 비활성 옵션 확인
- [ ] `14-M05b` 변신 규율: 절대적으로 혐오함 — 타인 어빌리티/투사체로 강제 변신 성공 확인
- [ ] `14-M06` 변신 규율: 못마땅함 → 변신 시 기분 -5, 의견 -10, 기억 감정 -5 (3일), **목격 기억 감정 -4 (3일)**
- [ ] `14-M06a` 변신 규율: 신경쓰지 않음 → 아무 효과 없음 (중립 단계)
- [ ] `14-M07` 변신 규율: 중시함 → 변신 시 기분 +5, 의견 +10, **목격 기억 감정 +4 (3일)**
- [ ] `14-M08` 변신 규율: 숭고함 → 변신 시 기분 +10, 의견 +20, **목격 기억 감정 +8 (5일)**
...
## 27. Fable 감사 수정 검증 (FABLE_AUDIT A~J)

### [M] 수동 확인 — A. 무기/의류 증발 방지
- [ ] `27-M01` 무기 락 폼(BearForm 등, weaponsOnTransform=Drop) + 근처 바닥 무기: 콜로니스트가 자동 픽업 Job을 **아예 받지 않음** (CanEquip 사전 차단)
- [ ] `27-M02` (백스톱) 락 상태에서 강제로 Equip Job 발생 시 무기가 증발하지 않고 폰 옆 바닥에 남음
- [ ] `27-M03` 의류 락 폼 + 맵에 더 좋은 옷: OptimizeApparel 헛걸음/거부 메시지 반복 없음
- [ ] `27-M04` 아웃핏 스탠드의 의류를 락 폰이 입으려 할 때 의류가 증발하지 않음

### [M] 수동 확인 — B/C. 변신 거부 시 Compat 무결성
- [ ] `27-M05` (FA 설치) 종족 불일치 폰에게 변신 투사체/약물 → 거부 후 **FA 얼굴이 원본 유지** (폼 얼굴 고착 없음)
- [ ] `27-M06` (SS 설치) 변신 거부된 폰의 사이드암 기억이 유지됨 (rememberedWeapons 소실 없음)
- [ ] `27-M07` (SS 설치) 정상 변신→해제 후 사이드암 기억 복원 (기존 동작 회귀 없음)

### [M] 수동 확인 — D/H/J. 상태 무결성
- [ ] `27-M08` 기본 장비설정(Keep) 폼으로 변신 중 세이브→로드: "prev apparel(s) could not be resolved" 경고 **없음**
- [ ] `27-M09` 로드 후 변신 해제: 이전 장비 추적/재착용 정상
- [ ] `27-M10` transformEnterFxDelayTicks 있는 폼(PhantomForm): 변신 직후 delay 내 해제 → 지연 Enter FX가 재생되지 **않음**
- [ ] `27-M11` 지연 Exit FX는 해제 후 정상 재생 (전환 시 이전 폼 Exit도 정상)

### [M] 수동 확인 — E/G. 표시/정리
- [ ] `27-M12` 같은 종족에게 과거에 입은 옛 상처가 변신 폰의 공격 후에도 원래 라벨 유지 (폼 이름으로 소급 재라벨 없음)
- [ ] `27-M13` 변신 폰을 데브 도구로 Destroy → 로그 에러 없음 (레지스트리 정리)
