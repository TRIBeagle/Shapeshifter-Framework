...
### 비주얼
- [ ] `[M]` 곰 텍스처 + bodyDrawScale 2.5 + 기본색 (112,82,65)
...
- [ ] `[M]` revertDrops: 해제 시 아이템 드랍 확인
- [ ] `[M]` Revert 기즈모 수동 해제 가능
- [ ] `[M]` 해제 후 텍스처/체형/스탯/hediff 전부 원복
- [ ] `[M]` 다른 폼 상태에서 스크롤/약물로 덮어쓰기 가능
...
- [ ] `[M]` revertAddHediffs: 해제 시 hediff 부여 확인 (바닐라 수명, 프레임워크 비추적)
...
### 비주얼
- [ ] `[M]` 렌더노드: 머리에 FloppyEars(Skin색), 몸에 FurryTail(Hair색)
...
### 수동 확인
- [ ] `[M]` 보이스: call/death/wounded/angry/eating/melee 커스텀 사운드
- [ ] `[M]` Verb 기즈모: 돌격소총 자동공격 ON / 나머지 4종 OFF + 커스텀 아이콘/라벨
...
### 체이닝
- [ ] `[M]` 인간 상태에서 진입 불가 (어빌리티 없음)
- [ ] `[M]` 수인(BeastkinForm) 상태에서만 Ability_FullBeast 표시 + 진입 가능
- [ ] `[M]` 수인 해제 시 FullBeast 어빌리티 제거
...
### 앰비언트 VFX
- [ ] `[M]` ambientFleck: 지정 간격마다 Fleck 스폰 확인
- [ ] `[M]` 맵 밖(캐러밴) → VFX 미재생 + 에러 없음
- [ ] `[M]` 세이브/로드 후 앰비언트 자동 재생성

### Auto-Verify
> `[A]` linkedHediff 스탯(이동+0.3, 피해배율x0.85), timer(12000틱)

### 수동 확인
- [ ] `[M]` revertOnDowned=true → 의식 상실 시 자동 해제
...
### 뮤턴트 필터 (Anomaly)
- [ ] `[M]` formAllowedMutants → 해당 뮤턴트만 변신 가능
- [ ] `[M]` formDisallowedMutants → 해당 뮤턴트 차단
- [ ] `[M]` 비-어빌리티 경로에서도 뮤턴트 필터 작동
...
### 어빌리티 경로
- [ ] `[M]` AoE 투사체: 사거리 25 + 반경 5칸 + 60% 양 변신
- [ ] `[M]` CursedArrow 투사체: 40% 양 변신

### 아이템/약물
- [ ] `[M]` ShiftScroll_Self: 사용 후 파괴, stackLimit 5
- [ ] `[M]` ShiftScroll_Target: 타겟 선택 UI + 이동 후 사용
- [ ] `[M]` BearElixir: DrugLab 제작, 복용 120틱
- [ ] `[M]` MagicStone: 인벤토리 소지 (소비 안 됨)
...
- [ ] `[M]` Hediff_RacialAwakening → RaceLocked 어빌리티
...
- [ ] `[M]` healthThreshold: 체력 30% 미만 → 자동 변신
- [ ] `[M]` triggerMentalStates: Berserk 진입 → 트리거
- [ ] `[M]` triggerSunGlowBelow: 밝기 0.5 미만(밤) → 트리거
- [ ] `[M]` triggerInCombat: 징집/피격 + 적 근처 → 트리거 (NPC 포함)
- [ ] `[M]` 이미 변신 중 → 재트리거 건너뜀
- [ ] `[M]` successChance < 1.0 → 확률적 발동
- [ ] `[M]` triggerOnce=true → 발동 후 hediff 제거
- [ ] `[M]` triggerOnce=false → 해제 후 재트리거 가능
- [ ] `[M]` checkIntervalTicks 간격 정상 (60 vs 120 vs 240)
...
- [ ] `[M]` 카라반/동면관/수송포드 중 변신 유지
- [ ] `[M]` 인공팔/결손팔에 BeastArm ForceAdd 정상

### 해제 부산물
- [ ] `[M]` revertDrops: despawned 폰 → 드랍 건너뜀 + 에러 없음
- [ ] `[M]` revertAddHediffs: 사망 폰 → 부여 건너뜀 + 에러 없음
- [ ] `[M]` addHediffs(추적)와 revertAddHediffs(비추적) 동시 사용 시 독립 작동

### 앰비언트 VFX
- [ ] `[M]` ambientEffecter 설정 폼 → 변신 중 Effecter 지속 재생
- [ ] `[M]` ambientEffecter 해제 시 Cleanup → 잔상 없음
- [ ] `[M]` 세이브/로드 후 ambientEffecter 자동 재생성
...
- [ ] `[M]` AddedPart/Hediff 실패 시 `[SSF]` 경고 로그
- [ ] `[M]` 장시간 플레이 후 메모리 누적 없음
