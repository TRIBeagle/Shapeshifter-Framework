...
|------|------|------|------|-----------|------|
| 0 | 절대적으로 혐오함 | -10 | -20 | -10 (5일) | **자기 주도 변신 금지** |
| 1 | 못마땅함 | -5 | -10 | -5 (3일) | - |
| - | 신경쓰지 않음 | - | - | - | 효과 없음 |
| 2 | 중시함 | +5 | +10 | - | - |
| 3 | 숭고함 | +10 | +20 | - | - |

> "절대적으로 혐오함" 단계에서는 **자기 주도** 변신이 차단됩니다: 어빌리티 기즈모 비활성, 약물 섭취 메뉴 비활성, 주문서/아이템 사용 불가. **타인에 의한 강제 변신**(타인 어빌리티, 투사체, 수술 약물 투여)은 허용되며, 변신은 정상 진행되고 감정 페널티만 적용됩니다.
...
| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `formDef` | `ShapeshiftFormDef` | null | 대상 폼. null = 런타임에 코드로 지정 (고급/디버그; 보통 XML에 지정). |
