...
// 용도 : - CompPostTick에서 주기적으로 체력/정신상태/밤/전투 조건을 검사.
...
namespace ShapeshifterFramework.Hediffs
{
    ...
    public class HediffComp_AutoShift : HediffComp
    {
        ...
        {
            // 체력 조건
            if (Props.healthThreshold > 0f
                && pawn.health?.summaryHealth != null
                && pawn.health.summaryHealth.SummaryHealthPercent < Props.healthThreshold)
                return true;
            ...
            // 밤 조건 (SunGlow 기반, 바이옴/계절 자동 반영)
            if (Props.triggerAtNight && pawn.Spawned && pawn.Map != null)
            {
                if (GenCelestial.CurCelestialSunGlow(pawn.Map) < 0.5f)
                    return true;
            }
