...
// 용도 : Pawn_MeleeVerbs.TryGetUpdatedMeleeVerb()는 pawn.verbTracker에서만 verb를 수집하기 때문에
//        shapeshiftVerbTracker의 폼 근접 도구가 선택되지 않음.
//        이 Postfix에서 replaceNativeTools=true이면 폼 도구로 강제 교체,
...
{
    [HarmonyPatch(typeof(Pawn_MeleeVerbs), nameof(Pawn_MeleeVerbs.TryGetUpdatedMeleeVerb))]
    internal static class Patch_Pawn_MeleeVerbs_TryGetUpdatedMeleeVerb
    {
        static void Postfix(Pawn_MeleeVerbs __instance, ref Verb __result, Thing target)
        {
            ...
            {
                Log.Warning($"[SSF] TryGetUpdatedMeleeVerb Postfix failed: {e}");
            }
        }
